...
const { generateFiles, parseFiles, cleanUp } = require('../file/file-api');
...
const commands = {
	clean: {
        method: (args) => {
			cleanUp();
			cliIo.printNewLines();
			cliIo.printMessage('SUCCESS: The data files and directory have been deleted.');
			cliIo.printNewLines();
		},
		validate: (args) => {
		...
		public: true,
        description: 'deletes all data files and the data-file directory from disk',
		usage: 'clean'		
	},
    gen: {
        method: (args) => {
    ...
    print: {
		...
		usage: 'print [-s gender|birth|name]'
	},
	printError: {
		method: (errorMessage) => {
